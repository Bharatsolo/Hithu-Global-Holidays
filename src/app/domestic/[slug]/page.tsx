...
export function generateStaticParams() {
    return domesticPackages.map((pkg) => ({
        slug: pkg.slug,
    }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const pkg = domesticPackages.find((p) => p.slug === slug);
    if (!pkg) return {};
    return {
...
export default async function DomesticPackageDetail({ params }: Props) {
    const { slug } = await params;
    const pkg = domesticPackages.find((p) => p.slug === slug);
    if (!pkg) notFound();
    ...
    const relatedPackages = domesticPackages
            ...
            <section className="relative h-72 md:h-[28rem] overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 md:p-12">
                    <div className="max-w-7xl mx-auto">
                        <AnimatedSection>
                            <Link href="/domestic" className="text-white/70 text-sm hover:text-white mb-2 inline-block">
                                ← Back to Domestic Packages
                            </Link>
                            <h1 className="font-heading font-extrabold text-3xl md:text-5xl text-white mb-3">
                                {pkg.title}
                            </h1>
                            ...
                            <AnimatedSection>
                                ...
                                <div className="flex flex-wrap gap-2 mt-4">
                                    {pkg.highlights.map((h, i) => (
                                        <span key={i} className="text-sm bg-primary/5 text-primary px-3 py-1.5 rounded-lg font-medium">
                                            {h}
                                        </span>
                                    ))}
                                </div>
                                ...
                                <AnimatedSection>
                                    <div className="bg-card rounded-2xl border border-border p-6 shadow-sm">
                                        ...
                                        <p className="text-text-light text-sm mb-4">
                                            Get a personalized quote and full details from our travel expert.
                                        </p>
                                        <a
                                            href={whatsappUrl}
                                            target="_blank"
                                        ...
                                        <Link
                                            href="/contact"
                                            className="w-full block mt-3 text-center px-6 py-3 bg-primary/5 text-primary rounded-xl font-semibold hover:bg-primary/10 transition-colors"
                                        >
                                            Send Enquiry Form
                                        </Link>
                                    </div>
                                    ...
                                    <div className="bg-card rounded-2xl border border-border p-6 shadow-sm space-y-4">
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex justify-between text-sm">
                                                <span className="text-text-lighter">Duration</span>
                                                <span className="text-text font-medium">{pkg.duration}</span>
                                            </div>
                                            <div className="flex justify-between text-sm">
                                                <span className="text-text-lighter">Ideal For</span>
                                                <span className="text-text font-medium">{pkg.idealFor}</span>
                                            </div>
                                            <div className="flex justify-between text-sm">
                                                <span className="text-text-lighter">Best Time</span>
                                                
                                            </div>
                                            <div className="flex justify-between text-sm">
                                                <span className="text-text-lighter">State</span>
                                                <span className="text-text font-medium">{pkg.state}</span>
                                            </div>
                                        </div>
                                    </div>
                                </AnimatedSection>
                ...
                <div className="max-w-3xl mx-auto text-center">
                    <AnimatedSection>
                        <h2 className="font-heading font-bold text-2xl md:text-3xl text-text mb-4">
                            Ready to Explore {pkg.state}?
                        </h2>
                        <p className="text-text-light mb-6">
                            Speak to our travel expert and get a customized itinerary tailored to your preferences.
                        </p>
                        <a
                            href={whatsappUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-2 px-8 py-4 bg-primary text-white rounded-xl font-bold text-lg hover:bg-primary-dark transition-colors"
                        >
